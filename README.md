...
| Vues | `0` iso · `1` face · `2` arrière · `3` dessus · `4` dessous · `5` gauche · `6` droite |
...
```bash
.venv\Scripts\python tests\make_sample.py     # régénère tests/sample.step (2 corps, couleurs corps + faces) et tests/box.step
...
- **Couleurs** : le STEP porte des valeurs sRGB. Le viewer HTML les convertit en linéaire (`Color.setRGB(..., SRGBColorSpace)`) ;
  OCP le fait lui-même (`Quantity_TOC_sRGB` pour les couleurs de l'interface).
